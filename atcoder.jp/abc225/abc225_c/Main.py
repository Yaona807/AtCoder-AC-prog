...
def main():
    ...
    for i in range(n):
        b = inputter(is_list=True,split=True)
        B.append(b)
    ...
    for i in range(n):
        for j in range(m):
            ...
            if 0 <= i+right_index[0] < n and 0 <= j+right_index[1] < m:
                if B[i][j] != B[i+right_index[0]][j+right_index[1]] - 1:
                    print('No')
                    return

            if 0 <= i+down_index[0] < n and 0 <= j+down_index[1] < m:
                if B[i][j] != B[i+down_index[0]][j+down_index[1]] - 7:
                    print('No')
                    return
