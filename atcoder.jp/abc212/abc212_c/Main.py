...
def main():
    n,m = inputter()
    a_list = inputter(is_list=True,split=True)
    b_list = inputter(is_list=True,split=True)
    new_a_list = sorted(a_list)
    new_b_list = sorted(b_list)

    ans = 1010101010101010101
    for i in new_a_list:
        ok = 0
        ng = m-1
        while abs(ok-ng)>1:
            ...
            if i >= new_b_list[mid]:
                ...
                ng = mid
        ans = min([ans,abs(i-new_b_list[ok]),abs(i-new_b_list[ng])])
    print(ans)
